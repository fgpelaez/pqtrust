...
| **2** (weeks 4–6) | PKCS#10 CSR flow + PKCS#8 export (delivered 2026-09-12), SLH-DSA (delivered 2026-09-16), `pqtrust` CLI, composite (hybrid) certs per `draft-ietf-lamps-pq-composite-sigs`, Dockerfile + compose | End-to-end demo: CSR → cert → hybrid-TLS handshake |
